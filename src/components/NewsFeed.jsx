...
export default function NewsFeed({ league, currentUser, isCommissioner }) {
  ...
  const load = useCallback(async () => {
    try {
      const data = await db.newsItems.list(league.id);
      setItems(data);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    setLoading(true);
    load();
    const unsubscribe = realtime.subscribeToNews(league.id, () => load());
    return unsubscribe;
  }, [load, league.id]);
  ...
  return (
    <section className="panel panel--tracker panel--full-width">
      <div className="panel__header">
        <h2 className="panel__title">News Feed</h2>
      ...
      <div className="panel__body">
        {loading ? (
          ...
          <p className="loading-text">
            No news yet{isCommissioner ? ' — post the first update below.' : '.'}
          </p>
        ) : (
            ...
            {!showAddForm ? (
              ...
              <form onSubmit={handleAdd} className="team-add-form">
                ...
                <input
                  ...
                  placeholder="Trade deadline extended"
                  required
                />
